      ...
      <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-200">
        ...
        <div v-if="isLoading" class="flex justify-center items-center py-20">
          <div class="flex flex-col items-center">
            ...
            <p class="mt-4 text-slate-600 font-medium">Chargement des univers...</p>
          </div>
        </div>

        <!-- Table -->
        <div v-else-if="filteredUniverses.length > 0" class="overflow-x-auto">
...
<!--                          </svg>-->
        ...
        <div v-else class="text-center py-20">
          ...
          >
            Ajouter le premier univers
          </button>
        </div>

        <!-- Pagination -->
        <div v-if="filteredUniverses.length > 0" class="bg-slate-50 px-6 py-4 border-t border-slate-200">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="text-sm text-slate-700">
              Affichage de <span class="font-semibold">{{ startIndex + 1 }}</span> à
              <span class="font-semibold">{{ Math.min(endIndex, filteredUniverses.length) }}</span>
              sur <span class="font-semibold">{{ filteredUniverses.length }}</span> résultats
            </div>
      ...
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full">
        <div class="p-6">
          ...
          <p class="text-slate-600 text-center mb-6">
            Êtes-vous sûr de vouloir supprimer l'univers <strong>{{ universeToDelete?.name }}</strong> ?
...
const sortKey = ref<keyof UniverseEntry>('name');
...
const loadUniverse = async () => {
  try {
    ...
    universes.value = await fetchUniverseData();
...
const totalPages = computed(() =>
  Math.ceil(filteredUniverses.value.length / itemsPerPage.value)
);
...
const paginatedUniverse = computed(() =>
  filteredUniverses.value.slice(startIndex.value, endIndex.value)
);

// Watchers
watch(filteredUniverses, () => {
  currentPage.value = 1;
});
...
// Méthodes de tri
const sort = (key: keyof UniverseEntry) => {
